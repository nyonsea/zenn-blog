...
本記事では、まずコンテナという技術そのものの正体（起動の仕組み・プロセスモデル）を掘り下げ、そのうえでネットワークの通信経路を整理し、続いてそれがクラウド（AWS）やオンプレでどう扱われるかを見ていきます。そして最後に、「概念の理解が机上の空論で終わっていないか」を検証する意味で、実務でよくある要件——オンプレの仮想化基盤上に、RHEL + Podmanのサーバーを100台規模でIaC展開する——を題材に、ここまでの理解を実際のコードに落とし込みます。対象読者は、コンテナをある程度触ったことがあり、次のレベルの理解を求めているエンジニアです。「とりあえず動かしてみた」系の内容ではありません。

## 1. コンテナ起動は「ミニLinuxの起動」ではない
...
## 2. 同梱ライブラリは「別プロセス」として起動するわけではない
...
## 11. 実践編：オンプレRHEL + Podman環境を、IaCで100台規模に展開する

ここまで、Docker・Kubernetes・ECS/EKS・Fargateと、コンテナ技術の主要な位置づけを一通り整理してきました。最後に、これらの理解が実際の構築作業でも通用することを確認する意味で、実務でよくある要件を1つ最後まで通してみます。10章で述べた通り、Dockerの挙動自体はインフラの種類を問わないため、**オンプレの仮想化基盤（VMware vSphereまたはRed Hat Virtualization）上にRHELサーバーを100台用意し、それぞれでPodmanコンテナを起動する**という一見AWSの話から離れたケースであっても、これまでの知識がそのまま活きることを示します。

...
本記事では、まずコンテナという技術そのものの正体（起動の仕組み・プロセスモデル）を掘り下げ、そのうえでネットワークの通信経路を整理し、最後にそれがクラウド（AWS）やオンプレでどう扱われるかを見ていきます。対象読者は、コンテナをある程度触ったことがあり、次のレベルの理解を求めているエンジニアです。「とりあえず動かしてみた」系の内容ではありません。
